...
#rd-sizer{display:none;align-items:center;gap:5px}
#rd-sizer label{font-size:9px;color:var(--txt3);white-space:nowrap}
#rd-sizer input[type=range]{width:90px;height:3px;accent-color:var(--hl);cursor:pointer}
/* ── メインエリア ── */
...
<div id="leg">
  ...
  <div id="rd-sizer">
    <label>&#9711; R</label>
    <input type="range" id="rd-radius-slider"
      min="200" max="1200" step="50" value="420"
      oninput="setRdRadius(+this.value)">
  </div>
  <div id="leg-sizer">
    <label>&#8596; TT</label>
...
function setLayout(mode) {
  ...
  if (mode === "rd") {
    ...
    if (rdSizer) rdSizer.style.display = "none";
  }
  update(root);
...
function init() {
  ...
  const _rdSizer = document.getElementById("rd-sizer");
  if (layoutMode === "rd") {
    lay.size([2 * Math.PI, _rdRadius]);
    if (_rdSizer) _rdSizer.style.display = "flex";
  } else {
    lay.nodeSize(layoutNodeSize());
    if (_rdSizer) _rdSizer.style.display = "none";
  }
  document.getElementById("btn-icon").classList.toggle("active", showIcons);
...
// 円形レイアウトの半径制御
let _rdRadius = parseInt(localStorage.getItem("taxa_rd_r") || "420");
function setRdRadius(r) {
  _rdRadius = r;
